 ...
 */
export async function initializeGlobalCollections(): Promise<void> {
  ...
  try {
    ...
    if (!settingsDoc.exists) {
      ...
      await logger.info(LogCategory.SYSTEM, 'Global collections initialized successfully');
    }
  } catch (error) {
    await logger.error(LogCategory.SYSTEM, 'Failed to initialize global collections', error as Error);
    throw error;
  }
}
 ...
 */
async function createGlobalCollections(db: admin.firestore.Firestore): Promise<void> {
  ...
  ranks.forEach(rank => {
    const rankRef = db.collection(collections.RANKS).doc(rank.rankName);
    batch.set(rankRef, {
      ...rank,
      createdAt: admin.firestore.FieldValue.serverTimestamp(),
      updatedAt: admin.firestore.FieldValue.serverTimestamp()
 ...
 */
export async function initializeUserCollections(uid: string): Promise<void> {
  const db = admin.firestore();
  ...
  try {
    ...
    const withdrawalRef = db.collection(collections.WITHDRAWALS).doc(`wd_${uid}_01`);
    ...
    const payoutRef = db.collection(collections.PAYOUT_QUEUE).doc(`pq_${uid}_01`);
    batch.set(payoutRef, {
      queueId: `pq_${uid}_01`,
      userId: uid,
      amount: 0,
    ...
    await logger.info(LogCategory.SYSTEM, `User-specific collections initialized for user: ${uid}`);
    
  } catch (error) {
    await logger.error(LogCategory.SYSTEM, 'Failed to initialize user collections', error as Error);
    throw error;
  }
}