...
export class SeedService {
  ...
  async seedAll(): Promise<void> {
    try {
      await logger.info(LogCategory.SYSTEM, 'Starting complete database seeding');
      ...
      await logger.info(LogCategory.SYSTEM, 'Complete database seeding finished');
    } catch (error) {
      await logger.error(LogCategory.SYSTEM, 'Complete seeding failed', error as Error);
      throw error;
    }
  }
  ...
  async seedRanks(): Promise<void> {
    try {
      await logger.info(LogCategory.SYSTEM, 'Seeding ranks');
      ...
      const ranks = [
        ...
        },
        {
          id: 'Starter',
          name: 'Starter',
          activationAmount: mlmConfig.ranks.azurite.activationAmount,
          benefits: {
            referralIncome: mlmConfig.incomes.referral.percentage,
            levelIncome: mlmConfig.incomes.level.L1, // Using L1 level income
            globalIncome: mlmConfig.incomes.global.percentage,
            retopupIncome: mlmConfig.incomes.reTopup.percentage,
            maxWithdrawal: 1000
          },
          requirements: {
            businessVolume: 0,
        ...
        },
        {
          id: 'Basic',
          name: 'Basic',
          activationAmount: mlmConfig.ranks.pearl.activationAmount,
          benefits: {
             referralIncome: mlmConfig.incomes.referral.percentage,
             levelIncome: mlmConfig.incomes.level.L1, // Using L1 level income
             globalIncome: mlmConfig.incomes.global.percentage,
        ...
        },
        {
          id: 'Standard',
          name: 'Standard',
          activationAmount: mlmConfig.ranks.ruby.activationAmount,
          benefits: {
            referralIncome: mlmConfig.incomes.referral.percentage,
            levelIncome: mlmConfig.incomes.level.L1, // Using L1 level income
            globalIncome: mlmConfig.incomes.global.percentage,
        ...
        },
        {
          id: 'Advanced',
          name: 'Advanced',
          activationAmount: mlmConfig.ranks.emerald.activationAmount,
          benefits: {
            referralIncome: mlmConfig.incomes.referral.percentage,
            levelIncome: mlmConfig.incomes.level.L1, // Using L1 level income
            globalIncome: mlmConfig.incomes.global.percentage,
        ...
        },
        {
          id: 'Professional',
          name: 'Professional',
          activationAmount: mlmConfig.ranks.sapphire.activationAmount,
          benefits: {
            referralIncome: mlmConfig.incomes.referral.percentage,
            levelIncome: mlmConfig.incomes.level.L1, // Using L1 level income
            globalIncome: mlmConfig.incomes.global.percentage,
        ...
        },
        {
          id: 'Executive',
          name: 'Executive',
          activationAmount: mlmConfig.ranks.diamond.activationAmount,
          benefits: {
            referralIncome: mlmConfig.incomes.referral.percentage,
            levelIncome: mlmConfig.incomes.level.L1, // Using L1 level income
            globalIncome: mlmConfig.incomes.global.percentage,
        ...
        },
        {
          id: 'Crown',
          name: 'Crown',
          activationAmount: mlmConfig.ranks.crown.activationAmount,
          benefits: {
            referralIncome: mlmConfig.incomes.referral.percentage,
            levelIncome: mlmConfig.incomes.level.L1, // Using L1 level income
            globalIncome: mlmConfig.incomes.global.percentage,
        ...
        },
        {
          id: 'Ambassador',
          name: 'Ambassador',
          activationAmount: mlmConfig.ranks.royalCrown.activationAmount,
          benefits: {
            referralIncome: mlmConfig.incomes.referral.percentage,
            levelIncome: mlmConfig.incomes.level.L1, // Using L1 level income
            globalIncome: mlmConfig.incomes.global.percentage,
      ...
      ranks.forEach(rank => {
        const rankRef = admin.firestore().collection(collections.RANKS).doc(rank.id);
        batch.set(rankRef, {
          ...rank,
      ...
      await batch.commit();
      await logger.info(LogCategory.SYSTEM, 'Ranks seeded successfully');

    } catch (error) {
      await logger.error(LogCategory.SYSTEM, 'Rank seeding failed', error as Error);
      throw error;
    }
  }
  ...
  async seedSettings(): Promise<void> {
    try {
      await logger.info(LogCategory.SYSTEM, 'Seeding settings');
      ...
      const settings = {
        ...
        income: {
          ...
          level: {
             percentage: mlmConfig.incomes.level.L1, // Using L1 as base percentage
          ...
          retopup: {
             percentage: mlmConfig.incomes.reTopup.percentage,
             enabled: true
           }
        },
        globalCycle: {
          targetAmount: mlmConfig.globalCycle.targetAmount,
          triggerInterval: mlmConfig.globalCycle.triggerInterval,
      ...
      Object.entries(settings).forEach(([category, data]) => {
        const settingRef = admin.firestore().collection(collections.SETTINGS).doc(category);
      ...
      await batch.commit();
      await logger.info(LogCategory.SYSTEM, 'Settings seeded successfully');

    } catch (error) {
      await logger.error(LogCategory.SYSTEM, 'Settings seeding failed', error as Error);
      throw error;
    }
  }
    ...
    try {
      ...
      for (const user of testUsers) {
        try {
          ...
          if (user.isAdmin) customClaims.admin = true;
          if (user.isSuperAdmin) customClaims.superAdmin = true;
          ...
          if (authError.code === 'auth/uid-already-exists') {
            await logger.info(LogCategory.SYSTEM, `User ${user.uid} already exists, skipping`);
            continue;
          }
          throw authError;
        }
      ...
      await logger.info(LogCategory.SYSTEM, 'Test users seeded successfully');

    } catch (error) {
      await logger.error(LogCategory.SYSTEM, 'Test users seeding failed', error as Error);
      throw error;
    }
  }
  ...
  private async seedTestTransactions(users: any[]): Promise<void> {
    try {
      ...
      for (const user of users.filter(u => u.isActive)) {
        ...
        const activationTxRef = admin.firestore().collection(collections.TRANSACTIONS).doc();
        batch.set(activationTxRef, {
          userId: user.uid,
          type: 'activation',
          rank: user.rank,
          amount: mlmConfig.ranks[user.rank as keyof typeof mlmConfig.ranks]?.activationAmount || 100,
          method: 'USDT BEP20',
          status: 'completed',
          details: {
        ...
        if (user.totalEarnings > 0) {
          ...
          incomeTypes.forEach(type => {
            const incomeRef = admin.firestore().collection(collections.INCOMES).doc();
            batch.set(incomeRef, {
              userId: user.uid,
              type,
      ...
      await logger.info(LogCategory.SYSTEM, 'Test transactions seeded successfully');

    } catch (error) {
      await logger.error(LogCategory.SYSTEM, 'Test transactions seeding failed', error as Error);
      throw error;
    }
  }
   ...
   * Seed global cycles
   */
  async seedGlobalCycles(): Promise<void> {
    try {
      ...
      const globalCycles = [
        {
          ...
          cycleNumber: 1,
          targetAmount: mlmConfig.globalCycle.targetAmount,
          currentAmount: mlmConfig.globalCycle.targetAmount,
          status: 'completed',
        ...
        },
        {
          ...
          cycleNumber: 2,
          targetAmount: mlmConfig.globalCycle.targetAmount,
          currentAmount: mlmConfig.globalCycle.targetAmount * 0.7,
          status: 'active',
      ...
      globalCycles.forEach(cycle => {
        const cycleRef = admin.firestore().collection(collections.GLOBAL_CYCLES).doc(cycle.id);
        batch.set(cycleRef, {
          ...cycle,
          updatedAt: admin.firestore.FieldValue.serverTimestamp()
        });
      ...
      await batch.commit();
      await logger.info(LogCategory.SYSTEM, 'Global cycles seeded successfully');

    } catch (error) {
      await logger.error(LogCategory.SYSTEM, 'Global cycles seeding failed', error as Error);
      throw error;
    }
  }
  ...
  async clearAllData(): Promise<void> {
    try {
      await logger.info(LogCategory.SYSTEM, 'Clearing all data');
      ...
      await logger.error(LogCategory.SYSTEM, 'Data clearing failed', error as Error);
      throw error;
    }
  }
  ...
  async isSeedComplete(): Promise<boolean> {
    try {
      const [ranksSnapshot, settingsSnapshot, usersSnapshot] = await Promise.all([
        admin.firestore().collection(collections.RANKS).limit(1).get(),
        admin.firestore().collection(collections.SETTINGS).limit(1).get(),
        admin.firestore().collection(collections.USERS).limit(1).get()
      ]);

      return !ranksSnapshot.empty && !settingsSnapshot.empty && !usersSnapshot.empty;

    } catch (error) {
      await logger.error(LogCategory.SYSTEM, 'Seed status check failed', error as Error);
      return false;
    }
  }
  ...
  async getSeedStatus(): Promise<any> {
    try {
      const [ranksCount, settingsCount, usersCount, transactionsCount, incomesCount, globalCyclesCount] = await Promise.all([
        admin.firestore().collection(collections.RANKS).count().get(),
        admin.firestore().collection(collections.SETTINGS).count().get(),
        admin.firestore().collection(collections.USERS).count().get(),
        admin.firestore().collection(collections.TRANSACTIONS).count().get(),
        admin.firestore().collection(collections.INCOMES).count().get(),
        admin.firestore().collection(collections.GLOBAL_CYCLES).count().get()
      ]);
      ...
      await logger.error(LogCategory.SYSTEM, 'Seed status fetch failed', error as Error);
      throw error;
    }
  }
}