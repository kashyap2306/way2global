...
exports.createDocumentTemplates = createDocumentTemplates;
exports.createAllUserDocuments = createAllUserDocuments;
exports.checkUserDocumentsExist = checkUserDocumentsExist;
exports.validateUserDocuments = validateUserDocuments;
exports.createRankTemplates = createRankTemplates;
    ...
    while (attempts < maxAttempts) {
        ...
        try {
            ...
            if (existingUser.empty) {
                await logger_1.logger.info(logger_1.LogCategory.SYSTEM, `Generated unique userCode: ${userCode}`);
                return userCode;
            }
            attempts++;
        }
        catch (error) {
            await logger_1.logger.error(logger_1.LogCategory.SYSTEM, 'Error checking userCode uniqueness', error);
            attempts++;
        }
    }
...
function createDocumentTemplates(uid, userCode, userData) {
...
async function createAllUserDocuments(uid, userData, sponsorId) {
    const db = admin.firestore();
    try {
        await logger_1.logger.info(logger_1.LogCategory.SYSTEM, `Starting document creation for user: ${uid}`);
        ...
        // Execute batch write
        await batch.commit();
        await logger_1.logger.info(logger_1.LogCategory.SYSTEM, `Successfully created all documents for user: ${uid} with userCode: ${userCode}`);
    }
    catch (error) {
        await logger_1.logger.error(logger_1.LogCategory.SYSTEM, `Failed to create documents for user: ${uid}`, error);
        // Attempt cleanup
...
async function checkUserDocumentsExist(uid) {
    const db = admin.firestore();
    try {
        ...
        await logger_1.logger.error(logger_1.LogCategory.SYSTEM, 'Error checking user document existence', error);
        return false;
    }
}
// Cleanup failed signup
async function cleanupFailedSignup(uid) {
    const db = admin.firestore();
    try {
        await logger_1.logger.info(logger_1.LogCategory.SYSTEM, `Cleaning up failed signup for user: ${uid}`);
        const batch = db.batch();
        ...
        await batch.commit();
        await logger_1.logger.info(logger_1.LogCategory.SYSTEM, `Cleanup completed for user: ${uid}`);
    }
    catch (error) {
        await logger_1.logger.error(logger_1.LogCategory.SYSTEM, 'Error during cleanup', error);
    }
}
// Validate all user documents exist
async function validateUserDocuments(uid) {
    const db = admin.firestore();
    try {
        ...
        for (const collection of requiredCollections) {
            ...
            if (!doc.exists) {
                await logger_1.logger.error(logger_1.LogCategory.SYSTEM, `Missing document in ${collection} for user: ${uid}`);
                return false;
            }
        }
        ...
        for (const docInfo of compoundDocs) {
            ...
            if (!doc.exists) {
                await logger_1.logger.error(logger_1.LogCategory.SYSTEM, `Missing document ${docInfo.id} in ${docInfo.collection} for user: ${uid}`);
                return false;
            }
        }
        await logger_1.logger.info(logger_1.LogCategory.SYSTEM, `All documents validated for user: ${uid}`);
        return true;
    }
    catch (error) {
        await logger_1.logger.error(logger_1.LogCategory.SYSTEM, 'Error validating user documents', error);
        return false;
    }
}
// Create rank templates (system initialization)
async function createRankTemplates() {
    const db = admin.firestore();
    try {
        const ranks = [
        ...
        await batch.commit();
        await logger_1.logger.info(logger_1.LogCategory.SYSTEM, 'Rank templates created successfully');
    }
    catch (error) {
        await logger_1.logger.error(logger_1.LogCategory.SYSTEM, 'Error creating rank templates', error);
    }
}
//# sourceMappingURL=userSignupService.js.map