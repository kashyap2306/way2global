...
async function processFundConversion(userData, paymentDetails, requiredAmount) {
    ...
    // Deduct from user's available balance for rank unlock
    const db = admin.firestore();
    await db.runTransaction(async (transaction) => {
        ...
        transaction.update(userRef, {
            availableBalance: admin.firestore.FieldValue.increment(-requiredAmount),
...
async function createActivationTransaction(userData, activationData) {
    ...
    try {
        const transactionData = {
            uid: userData.uid,
            type: 'rank_unlock', // Updated for pool-based system
            ...
            description: `Rank unlock to ${activationData.rank} rank`,
            // Timestamps
            ...
            completedAt: activationData.paymentMethod === 'fund_conversion'
                ...
                : null,
            // Metadata for pool-based system
            metadata: {
                previousRank: userData.rank,
                userLevel: userData.level,
                sponsorUID: userData.sponsorUID,
                autoGenerated: false,
                poolBased: true, // New flag for pool-based system
                directReferrals: userData.directReferrals || 0,
                claimEligible: userData.claimEligible || false
            }
        };
        const transactionRef = await db.collection(config_1.collections.TRANSACTIONS).add(transactionData);
        // If fund conversion (instant), update user rank immediately
        if (activationData.paymentMethod === 'fund_conversion') {
            await db.collection(config_1.collections.USERS).doc(userData.uid).update({
                rank: activationData.rank,
                rankActivatedAt: admin.firestore.FieldValue.serverTimestamp(),
                activationAmount: admin.firestore.FieldValue.increment(rankConfig.activationAmount),
                updatedAt: admin.firestore.FieldValue.serverTimestamp()
            });
        }
